...
class Jeu:
    ...
    def generer_tous(self) -> list[Domino]:
        ...
        c'est à dire, toutes les combinaisons unique de couple entre 0 et 6
        '''
        liste_dominos = []
        for i in range(0, 7):
            for j in range(i, 7):
    ...
    def dominos_posables(self) -> list[Domino]:
        ...
        for d in self.dominos_joueur:
            if self.dominos_poses[-1].accepte_apres(d) or self.dominos_poses[-1].accepte_apres(d.retourner()):
                d_posables.append(d)
        return d_posables
    ...
    def poser_domino(self, domino) -> None:
        ...
        if self.dominos_poses[-1].accepte_apres(domino):
            ...
            print("\n" + "*" * 20 + " WARNING " + "*" * 20)
            print("Le Domino suivant n'est pas posable : " + str(domino), end = ' ')
    ...
    def choix_joueur(self) -> Domino:
        ...
        domino = None
        while True:
            i = input('Indiquez le numéro du domino que vous souhaitez jouer (de 0 à ' + 
                       str(len(self.dominos_joueur) - 1) + ') --- ou p pour piocher --- ou q pour quitter : ')
