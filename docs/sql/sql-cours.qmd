...
SQL permet d'effectuer les opérations du CRUD (Create, Read, Upadate, Delete)
...
```{.sql}
INSERT INTO ran.personne (id, nom, prenom, dnais, adresse)
VALUES
...
SELECT p.prenom,
       ...
       c.quantite
  FROM ran.personne p
  JOIN ran.commande c ON p.id = c.id_personne
 WHERE prenom = 'Laure';
```
...
SELECT p.prenom,
       ...
       c.quantite
  FROM ran.personne p
  JOIN ran.commande c ON p.id = c.id_client
```
...
SELECT p.prenom,
       ...
       c.quantite
  FROM ran.personne p
  LEFT JOIN ran.commande c ON p.id = c.id_client
```
...
| id  | nom     | prenom | dnais      | adresse | joue_aux_echecs |
| --- | ------- | ------ | ---------- | ------- | --------------- |
