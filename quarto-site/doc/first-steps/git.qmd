...
- [ ] `git status` : elle affiche l'état du dépôt. Vous remarquez que le fichier *ex1.py* apparait dans les « Untracked files » i.e. les fichiers que Git a repéré mais ne versionne pas pour l'instant.
- [ ] `git add ex1.py` : indique à Git que ce fichier a été créé/modifié et qu'il faut l'ajouter au prochain commit
- [ ] Si vous faites de nouveau `git status`, le fichier apparait maintenant dans « Changes to be commited »
- [ ] `git commit -m "ajout du fichier ex1"` : pour créer un commit (point de sauvegarde). `-m` signifie que l'on ajoute ensuite un message (obligatoire) pour expliquer brievement ce que contient ce commit
- [ ] Faites encore une fois `git status`. Vous notez que votre dépôt local a un commit d'avance sur le dépôt distant.
- [ ] `git push` comme précisé pour pousser votre code vers le dépôt distant
...
- [ ] Sur la page GitHub de votre dépôt distant, créez ou modifiez un fichier
  - :warning: ce n'est pas une bonne pratique mais ici c'est pour l'exercice
- [ ] Sur votre dépôt local : `git pull`
...
[Introdution à Git](https://ludo2ne.github.io/ENSAI-tools/doc/git/presentation.html){target="_blank"}